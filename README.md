...
- **读取纪律**：本仪表看形态（方向、动量、z-score），不看水平绝对值。MEM 仅含两个成分（MU/SNDK），SNDK 带分拆上市定价修复的特殊路径，ratio 的绝对水平被个体性显著抬高——7.90 不携带"贵/便宜"信息。携带信息的是方向变化、动量和极端读数。
...
|---|---|---|
| #1 面板一+四渲染 | ✅ | 管线跑通，数据就绪 |
| #2 N1-N11 | ✅ 11/11 PASS | N2 N10 N11 全量覆盖 |
| #3 定位声明 | ✅ | index.html + README 双份 |
| #4 连续5日 Actions 无干预 | ⏳ 验收中 | 最早 2026-07-11 满足 |
| #5 维护手册 | ✅ | README §维护手册 |
| #6 ICS 订阅端实测 | ⚠️ 单元通过 | UID幂等=✅；Google端轮询验证待做 |
| #7 算术校验 (MEM 7.90) | ✅ 手工对账 | buy-and-hold 9.75 → 再平衡 7.899，精确吻合 |

### 三方工作流规则（2026-07-05 入档）

CodeBuddy 的 bug/修复状态报告必须携带 git 证据（commit hash 或 diff 片段），叙事性状态陈述（"X 修了但 Y 漏了"类）不作为审计输入。本次事故：一句"build_panels 漏修复"的错误状态报告触发了三轮无实际价值的污染链追查，最终 git 单 commit 证明两文件同时修复。若首次报告即带 hash（536ead9），三轮追查缩为一轮。虚惊换来了 N10/N11 防线，但同类虚惊第二次是纯成本。

### 已知技术债

- **ticker 解析收敛**：`fetch_prices.py` 和 `build_panels.py` 已通过共享 `loader.py` 收敛 ticker 列表，N10 兜底保证取数身份一致。

### 历史记录

- 2026-07-05：`set("SOXX")`→`{"S","O","X"}` bug 在两文件中同时修复（同一 commit），基准始终使用真实 SOXX 数据。上一轮报告声称"build_panels.py 漏修复"为事后审计时的叙事错误，已纠正。
